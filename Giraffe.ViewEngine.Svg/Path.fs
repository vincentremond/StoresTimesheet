...
module Path =
    ...
    type [<RequireQualifiedAccess>] PathCommand =
        | Move of Move
        | Line of Line
    ...
    and [<RequireQualifiedAccess>] CubicBezierCurve =
        ...
        static member cubic t x1 y1 x2 y2 x y = PathCommand.CubicBezierCurve (Cubic (t, x1, y1, x2, y2, x, y))
        static member smooth t x2 y2 x y = PathCommand.CubicBezierCurve (Smooth (t, x2, y2, x, y))
    ...
    and [<RequireQualifiedAccess>] QuadraticBezierCurve =
        ...
        static member quadratic t x1 y1 x y = PathCommand.QuadraticBezierCurve (Quadratic (t, x1, y1, x, y))
        static member smooth t x y = PathCommand.QuadraticBezierCurve (Smooth (t, x, y))
    ...
    and [<RequireQualifiedAccess>] EllipticalArc =
        | EllipticalArc of t: Type * rx: float *  ry: float * angle: float<deg> * largeArcFlag: LargeArcFlag * sweepFlag: SweepFlag * x: float * y: float
        
        static member arc t rx ry angle largeArcFlag sweepFlag x y = PathCommand.EllipticalArc (EllipticalArc (t, rx, ry, angle, largeArcFlag, sweepFlag, x, y))
        
        static member value =
            function
            | EllipticalArc (Absolute, rx, ry, angle, largeArcFlag, sweepFlag, x, y) -> $"A %f{rx},%f{ry} %f{angle} %d{LargeArcFlag.value largeArcFlag} %d{SweepFlag.value sweepFlag} %f{x},%f{y}"
            | EllipticalArc (Relative, rx, ry, angle, largeArcFlag, sweepFlag, x, y) -> $"a %f{rx},%f{ry} %f{angle} %d{LargeArcFlag.value largeArcFlag} %d{SweepFlag.value sweepFlag} %f{x},%f{y}"
